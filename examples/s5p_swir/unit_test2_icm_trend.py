...
def main():
    ...
    icm_mon_db = 'background-noise/swir/mon_backgroundnoise_swir'
    ...
    with ICMmonStability(data_dir + icm_mon_db) as mon:
        ...
        stats_list[-1].attrs['long_name'] = 'detector median of plain noise'
        stats_list.append(trend_stats(mon, orbit_bgn, orbit, ['noise_b1']))
        stats_list[-1].attrs['long_name'] = 'detector median of biweight noise'
        stats_list.append(trend_stats(mon, orbit_bgn, orbit, ['read_noise']))
        stats_list[-1].attrs['long_name'] = 'normalized read-noise'
    ...
    for stats in stats_list:
        ...
        if 1 == 0:
            indx = np.where((eng_hk['detector_temp'].values['err1'] > 139.95)
                            ...
                            & (eng_hk['grating_temp'].values['mean'] < 202.1))
            eng_hk = eng_hk.isel(orbit=indx[0], drop=True)

        title = f'trend of {stats.attrs["long_name"]} & temperatures'
        plot.draw_trend(stats, eng_hk, fig_info=fig_info_in.copy(),
                        title=title, vperc=[1, 99])
    plot.close()
